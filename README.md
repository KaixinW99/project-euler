...
<!-- Pick 3-5 from https://projecteuler.net/progress (Awards tab) and list them here, e.g.
- Award name — what it was for
Delete this section if you prefer not to list any. -->

## Layout

```
solutions/
  README.md        # index: problem number, title, link
  p001.py … p100.py  # one file per solved problem, each prints its answer
data/              # input files Project Euler provides for some problems
solved.txt         # my solved-problem IDs
progress.svg       # generated by tools/make_progress.py
tools/
  make_progress.py
```

Run any solution from the repository root, e.g. `python solutions/p054.py`.
Some solutions use `numpy`, `sympy`, `scipy` or `numba`.
